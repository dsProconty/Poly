...
export async function resolvePositions(supabase) {
  ...
  for (const position of openPositions) {
    ...
    // IMPORTANTE: solo inferir NO (precio bajo) si el end_date ya pasó.
    // Un precio bajo mientras el mercado sigue abierto no significa que perdió —
    // el evento aún puede ocurrir antes de la fecha límite.
    const endDatePassed = position.end_date
      ? new Date(position.end_date) < new Date()
      : false; // sin end_date guardado → no inferir NO
    ...
    if (marketData.resolved && marketData.outcome) {
      ...
      // YES definitivo (>95%): seguro que sí ocurrió, cerrar siempre
      if (marketData.yesPrice > 0.95) marketOutcome = 'YES';
      // NO por precio: solo si el mercado ya cerró su fecha límite
      else if (marketData.yesPrice < 0.05 && endDatePassed) marketOutcome = 'NO';
    }
