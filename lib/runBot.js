...
const MAX_MARKETS_PER_RUN = parseInt(process.env.MAX_MARKETS_PER_RUN || '3');
...
export async function runBot(supabase) {
  ...
  // Set de market_ids con posición abierta — evita duplicados
  ...
  for (const market of candidates) {
    ...
    // Evitar apuesta duplicada en el mismo mercado
    if (openMarketIds.has(market.marketId)) {
      log.push({ market: market.question, action: 'SKIP', reason: 'Ya hay una posición abierta en este mercado' });
      continue;
    }
