...
export async function runBot(supabase) {
  ...
  if (!markets.length) {
    return { status: 'no_markets', message: 'Sin mercados deportivos activos con volumen suficiente' };
  }
  ...
  for (const market of candidates) {
    ...
    // Búsqueda de contexto real del partido via Tavily
    const context = await searchMatchContext(market.question);
