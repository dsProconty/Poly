...
/**
 * Fetch principal — sin tag_slug (no es fiable), filtra por keywords.
 * Ordena por endDate para priorizar partidos que cierran pronto.
 */
export async function getSportsMarkets() {
  const res = await fetch(
    `${GAMMA_BASE}/markets?active=true&closed=false&archived=false&limit=100&order=endDate&ascending=true`
  );
  ...
  return (raw || [])
    ...
    .map(toMarket)
    .filter(m =>
      m.sport !== null &&                          // deporte identificado
      m.volume    > MIN_VOLUME        &&
...
/**
 * Fallback — amplía la ventana de días si el fetch principal da 0.
 * Aplica los mismos filtros de calidad, solo relaja la ventana temporal.
 */
export async function getSportsMarketsFromMarkets() {
  ...
  return (raw || [])
    ...
    .filter(m =>
      ...
      m.yesProb >= MIN_YES_PROB &&
      m.yesProb <= MAX_YES_PROB
      // sin filtro de fecha — acepta cualquier mercado próximo
    );
}
